...
import com.android.volley.self.CommonUtil;
import com.android.volley.self.ImageCropUtil;
...
public class ImageLoader {
	 ...
	 * @param ctx
	 */
	public static void getDefaultBitmap(int id, Context ctx) {
		if (null == sDefaultBitmap) {
			sDefaultBitmap = ImageCropUtil.toRoundBitmap(ImageCropUtil
					.decodeResource(ctx, id, CommonUtil.dip2px(ctx, 30),
							CommonUtil.dip2px(ctx, 30), ScaleType.FIT_XY));
		}
	}
	 ...
	 * @param ctx
	 */
	public static void getErrorBitmap(int id, Context ctx) {
		if (null == sErrorBitmap) {
			sErrorBitmap = ImageCropUtil.toRoundBitmap(ImageCropUtil
					.decodeResource(ctx, id, CommonUtil.dip2px(ctx, 30),
							CommonUtil.dip2px(ctx, 30), ScaleType.FIT_XY));
		}
	}
	 ...
	 * @return 设置圆角图片（圆形）
	 */
	public static ImageListener getImageCornerListener(final ImageView view,
			final int defaultImageResId, final int errorImageResId,
			final Context ctx) {
		...
		return new ImageListener() {
			...
			public void onErrorResponse(VolleyError error) {
				if (errorImageResId != 0) {
					getErrorBitmap(errorImageResId, ctx);
					view.setImageBitmap(sErrorBitmap);
				}
			}
			...
			public void onResponse(ImageContainer response, boolean isImmediate) {
				if (response.getBitmap() != null) {
					...
					sErrorBitmap = ImageCropUtil.toRoundBitmap(response
							.getBitmap());

					view.setImageBitmap(ImageCropUtil.toRoundBitmap(response
							.getBitmap()));

				} else if (defaultImageResId != 0) {
					getDefaultBitmap(defaultImageResId, ctx);
					view.setImageBitmap(sDefaultBitmap);
				}
			}
		};
	}
